...
#define SENSOR_UXGA_20FPS_MODE   4
...
extern void sensor_init_uxga_20fps();
...
static HI_S32 cmos_get_sensor_max_resolution(ISP_CMOS_SENSOR_MAX_RESOLUTION *pstSensorMaxResolution)
{
    ...
    pstSensorMaxResolution->u32MaxHeight = 1200;
...
static HI_U8  isp_image_mode_get(ISP_CMOS_SENSOR_IMAGE_MODE *pstSensorImageMode)
{
    ...
    else if((pstSensorImageMode->u16Width == 1920)&&(pstSensorImageMode->u16Height == 1200))
    {
        if(pstSensorImageMode->u16Fps == 20)
        {
            gu8SensorImageMode = SENSOR_UXGA_20FPS_MODE;
        }
        else
        {
            return -1;
...
HI_S32 cmos_set_image_mode(ISP_CMOS_SENSOR_IMAGE_MODE *pstSensorImageMode)
{
    HI_S32 s32Ret;
...
HI_S32 sensor_set_image_mode(ISP_CMOS_SENSOR_IMAGE_MODE *pstSensorImageMode)
{
    return cmos_set_image_mode (pstSensorImageMode);
}
