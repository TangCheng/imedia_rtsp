...
gpointer ipcam_ivideo_get_video_data(IpcamIVideo *self)
{
    ...
    return IPCAM_IVIDEO_GET_INTERFACE(self)->get_video_data(self);
}

void ipcam_ivideo_release_video_data(IpcamIVideo *self, gpointer data)
{
    g_return_if_fail(IPCAM_IS_IVIDEO(self));
    IPCAM_IVIDEO_GET_INTERFACE(self)->release_video_data(self, data);
}

gboolean ipcam_ivideo_has_video_data(IpcamIVideo *self)
{
    g_return_val_if_fail(IPCAM_IS_IVIDEO(self), FALSE);
    return IPCAM_IVIDEO_GET_INTERFACE(self)->has_video_data(self);
}

void ipcam_ivideo_register_rtsp_source(IpcamIVideo *self, void *source)
{
    g_return_if_fail(IPCAM_IS_IVIDEO(self));
    IPCAM_IVIDEO_GET_INTERFACE(self)->register_rtsp_source(self, source);
}

void ipcam_ivideo_unregister_rtsp_source(IpcamIVideo *self, void *source)
{
    g_return_if_fail(IPCAM_IS_IVIDEO(self));
    IPCAM_IVIDEO_GET_INTERFACE(self)->unregister_rtsp_source(self, source);
}
